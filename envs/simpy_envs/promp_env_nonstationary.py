...
class MetaEnv(Env):
    ...
    def step(self, action):
        if STATIONARY == False:
            if self.simpy_env.now//24 == 100:
                #print("Former_Scenario:", self.scenario)
                self.scenario = self.tasks[1]
                self.setting_scenario()
                #print("After_Scenario:", self.scenario)

            if self.simpy_env.now//24 == 150:
                #print("Former_Scenario:", self.scenario)
                self.scenario = self.tasks[2]
                self.setting_scenario()
                #print("After_Scenario:", self.scenario)
    ...
    def setting_scenario(self):
        for procurement in self.procurementList:
            procurement.lead_time = self.scenario["LEADTIME"]
        self.customer.demand_qty_dict = self.scenario["DEMAND"] 
